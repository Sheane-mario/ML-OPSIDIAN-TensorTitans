"""
V19 Excalibur Stacker

Reads all available checkpoints from the Excalibur training script, 
dynamically compiles them into a stacking matrix, and uses the 
HuberRegressor to generate the ultimate submission.

Since the base models were trained on transformed targets and naturally squashed
to [0, 1] during prediction, the stacking matrix is perfectly scaled.
"""
...
DATA_DIR = "data"
OUTPUT_FILE = "submissions/submission_v19_excalibur.csv"

os.makedirs("submissions", exist_ok=True)

print("=== V19 Excalibur Auto-Stacker ===")

oof_files = sorted(glob.glob(os.path.join(CHECKPOINT_DIR, "*_oof.npy")))
if not oof_files:
    print("No checkpoints found! Have you run `v19_excalibur_solution.py` yet?")
    exit(1)
...
# To handle any extremely minor floating point inaccuracies
...
print(f"\n[EXCALIBUR STACK] OOF RMSE = {rmse:.5f} | R2 = {r2:.4f}")
