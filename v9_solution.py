"""
V9 (no-optuna): MLP + KNN added to V8's proven ensemble.
Removes Optuna entirely - it was causing hangs.
Uses V8's best CatBoost params directly.
"""
...
# LGB2 (RMSE, 127L)
print("\n=== LightGBM (RMSE, 127L) ===")
oof['lgb2'] = np.zeros(len(X)); test_preds['lgb2'] = np.zeros(len(X_test))
lgb_p2 = {'objective':'regression','metric':'rmse','learning_rate':0.03,
           'num_leaves':127,'min_child_samples':15,'feature_fraction':0.75,
           'bagging_fraction':0.8,'bagging_freq':5,'reg_alpha':0.05,'reg_lambda':0.5,
           'n_jobs':-1,'verbose':-1,'seed':SEED+1}
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    m = lgb.train(lgb_p2, lgb.Dataset(X_arr[tr_i],y_arr[tr_i],feature_name=feature_cols),
                  5000, valid_sets=[lgb.Dataset(X_arr[val_i],y_arr[val_i])],
                  callbacks=[lgb.early_stopping(150,verbose=False),lgb.log_evaluation(9999)])
    oof['lgb2'][val_i] = m.predict(X_arr[val_i]); test_preds['lgb2'] += m.predict(X_test_arr)/N_FOLDS
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['lgb2'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['lgb2'][val_i]):.4f}")
print(f"LGB2 OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['lgb2'])):.5f} R2={r2_score(y_arr,oof['lgb2']):.4f}")
...
# CatBoost (V8's best params)
print("\n=== CatBoost (V8 params) ===")
oof['cat'] = np.zeros(len(X)); test_preds['cat'] = np.zeros(len(X_test))
cat_p = dict(iterations=5000,learning_rate=0.03,depth=8,l2_leaf_reg=5,
...
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    ...
    oof['cat'][val_i] = m.predict(X_arr[val_i]); test_preds['cat'] += m.predict(X_test_arr)/N_FOLDS
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['cat'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['cat'][val_i]):.4f}")
print(f"CAT OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['cat'])):.5f} R2={r2_score(y_arr,oof['cat']):.4f}")

# MLP Neural Network
print("\n=== MLP Neural Network ===")
oof['mlp'] = np.zeros(len(X)); test_preds['mlp'] = np.zeros(len(X_test))
for fold,(tr_i,val_i) in enumerate(kf.split(X_scaled)):
    layers = (256,128,64) if fold%2==0 else (128,64,32)
    m = MLPRegressor(hidden_layer_sizes=layers, alpha=0.05, learning_rate_init=0.001,
                     max_iter=500, random_state=SEED+fold,
                     early_stopping=True, validation_fraction=0.1, n_iter_no_change=20,
                     activation='relu', solver='adam')
    m.fit(X_scaled[tr_i], y_arr[tr_i])
    oof['mlp'][val_i] = m.predict(X_scaled[val_i]); test_preds['mlp'] += m.predict(X_test_scaled)/N_FOLDS
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['mlp'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['mlp'][val_i]):.4f}")
print(f"MLP OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['mlp'])):.5f} R2={r2_score(y_arr,oof['mlp']):.4f}")

# KNN (k=30)
print("\n=== KNN (k=30) ===")
oof['knn'] = np.zeros(len(X)); test_preds['knn'] = np.zeros(len(X_test))
for fold,(tr_i,val_i) in enumerate(kf.split(X_scaled)):
    m = KNeighborsRegressor(n_neighbors=30, weights='distance', n_jobs=-1)
    m.fit(X_scaled[tr_i], y_arr[tr_i])
    oof['knn'][val_i] = m.predict(X_scaled[val_i]); test_preds['knn'] += m.predict(X_test_scaled)/N_FOLDS
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['knn'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['knn'][val_i]):.4f}")
print(f"KNN OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['knn'])):.5f} R2={r2_score(y_arr,oof['knn']):.4f}")
...
# Blend (best models only — positive R2)
good_k      = [k for k, r2 in zip(model_names, r2_list) if r2 > 0.02]
good_rmse   = [rmse_list[model_names.index(k)] for k in good_k]
inv_r       = np.array([1/r for r in good_rmse]); wts = inv_r/inv_r.sum()
blend_oof   = sum(w*oof[k] for w,k in zip(wts,good_k))
blend_test  = sum(w*test_preds[k] for w,k in zip(wts,good_k))
...
for k, r, r2 in zip(model_names, rmse_list, r2_list):
    print(f"  {k:10s} RMSE={r:.5f} R2={r2:.4f} std={np.std(test_preds[k]):.4f}")
print(f"  {'Stack':10s} RMSE={stack_rmse:.5f} R2={stack_r2:.4f} std={np.std(stack_test):.4f}")
print(f"  {'Blend':10s} RMSE={blend_rmse:.5f} R2={blend_r2:.4f} std={np.std(blend_test):.4f}")
print(f"  Ridge coefs: {dict(zip(model_names, ridge.coef_.round(4)))}")

for fname, preds in [
    (OUTPUT_FILE,                stack_test),
    ("submission_v9_blend.csv",  blend_test),
    ("submission_v9_cat.csv",    test_preds['cat']),
]:
...
print("\n" + "="*60)
print("V9 SUMMARY (MLP + KNN + V8 base, no Optuna)")
print("="*60)
for k,r,r2 in zip(model_names,rmse_list,r2_list): print(f"  {k:10s} RMSE={r:.5f} R2={r2:.4f}")
