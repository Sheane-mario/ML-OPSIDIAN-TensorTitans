"""
V16: The "Shallow & Robust" Strategy

V15 (Huber stack) achieved our new best score of 0.38189!
Key insight from V15: The Huber meta-model heavily rewarded shallower trees
(Cat5=40%, Cat6=28%, Cat7=24%, Cat8=4%, Cat10=0%). 
The custom metric hates outliers, and deep trees overfit and create outliers.

V16 Strategy:
  - Exact same 115 features.
  - Shift the CatBoost depth sweep strictly SHALLOWER: depths 3, 4, 5, 6, 7.
  - Add multiple random seeds for the best depths (5 and 6) to reduce variance.
  - Use HuberRegressor for stacking, but try multiple epsilon values (1.20, 1.35).
    Lower epsilon = more robust to outliers.
"""
...
N_FOLDS     = 10
DATA_DIR    = "data"
OUTPUT_FILE = "submission_v16_huber_135.csv"
np.random.seed(SEED)
...
# LGB RMSE
print("\n=== LGB (RMSE) ===")
oof['lgb_rmse'] = np.zeros(len(X)); test_preds['lgb_rmse'] = np.zeros(len(X_test))
lgb_rmse_p = {'objective':'regression','metric':'rmse','learning_rate':0.03,
              'num_leaves':63,'min_child_samples':20,'feature_fraction':0.7,
              'bagging_fraction':0.8,'bagging_freq':5,'reg_alpha':0.1,'reg_lambda':1.0,
              'n_jobs':-1,'verbose':-1,'seed':SEED+10}
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    m = lgb.train(lgb_rmse_p, lgb.Dataset(X_arr[tr_i],y_arr[tr_i],feature_name=feature_cols),
                  5000, valid_sets=[lgb.Dataset(X_arr[val_i],y_arr[val_i])],
                  callbacks=[lgb.early_stopping(150,verbose=False),lgb.log_evaluation(9999)])
    oof['lgb_rmse'][val_i] = m.predict(X_arr[val_i]); test_preds['lgb_rmse'] += m.predict(X_test_arr)/N_FOLDS


# CatBoost SHALLOW sweep (depths 3, 4, 5, 6, 7) + multiple seeds for 5 & 6
cat_configs = [
    ('cat3',    3,  1.0, 5,   SEED),
    ('cat4',    4,  1.5, 8,   SEED),
    ('cat5_s1', 5,  2.0, 10,  SEED),
    ('cat5_s2', 5,  2.0, 10,  SEED+99),
    ('cat6_s1', 6,  3.0, 15,  SEED),
    ('cat6_s2', 6,  3.0, 15,  SEED+99),
    ('cat7',    7,  5.0, 20,  SEED),
]

for name, depth, l2, min_leaf, seed in cat_configs:
    print(f"\n=== CatBoost {name} ===")
...
# 1. Huber Regressor (Epsilon=1.35, Standard robust)
h135 = HuberRegressor(epsilon=1.35, alpha=0.0001, max_iter=1000)
h135.fit(oof_stack, y_arr)
h135_oof  = h135.predict(oof_stack)
h135_test = h135.predict(test_stack)
h135_rmse = np.sqrt(mean_squared_error(y_arr, h135_oof))

# 2. Huber Regressor (Epsilon=1.20, More robust to outliers)
h120 = HuberRegressor(epsilon=1.20, alpha=0.0001, max_iter=1000)
h120.fit(oof_stack, y_arr)
h120_oof  = h120.predict(oof_stack)
h120_test = h120.predict(test_stack)
h120_rmse = np.sqrt(mean_squared_error(y_arr, h120_oof))

print("\n=== Final Results ===")
for i, k in enumerate(model_names):
    r = np.sqrt(mean_squared_error(y_arr, oof[k]))
    print(f"  {k:10s} RMSE={r:.5f}  H135={h135.coef_[i]:.4f}  H120={h120.coef_[i]:.4f}")

print(f"\n  {'H135 Stack':10s} RMSE={h135_rmse:.5f} R2={r2_score(y_arr, h135_oof):.4f}")
print(f"  {'H120 Stack':10s} RMSE={h120_rmse:.5f} R2={r2_score(y_arr, h120_oof):.4f}")

for fname, preds in [
    (OUTPUT_FILE,                          h135_test),
    ("submission_v16_huber_120.csv",       h120_test),
]:
