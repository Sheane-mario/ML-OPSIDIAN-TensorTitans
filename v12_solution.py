"""
V12: V8's EXACT feature set (104 features) + CatBoost depth=6 swap

Key insight:
  - V8 (104 features, Cat depth=8) → LB 0.38215 (best!)
  - V10 (109 features, Cat depth=6) → LB 0.38240 (worse)
  - More features = more noise; V8's clean 104-feature set generalizes best
  - Cat depth=6 > depth=8 on OOF (0.23452 vs 0.23484)
  
V12: Take V8's proven feature set + swap Cat depth=8 → depth=6 + try 3 cat configs
"""
...
N_FOLDS     = 10
DATA_DIR    = "data"
OUTPUT_FILE = "submission_v12.csv"
np.random.seed(SEED)
...
# V8 EXACT cat cols (no road_quality or flood_occurrence TEs)
...
gm = y.mean()
print("Computing fold-safe TEs (V8 exact: place, district, soil, landcover)...")
# V8 EXACT TEs - only 4 columns, same smoothing
for col, smooth in [('place_name',5),('district',10),('soil_type',10),('landcover',10)]:
...
print(f"Final features: {len(feature_cols)} (V8 had 104)")
...
# CatBoost depth=5 (best solo depth from V11 sweep: RMSE=0.23458, R2=0.0348)
print("\n=== CatBoost depth=5 ===")
oof['cat5'] = np.zeros(len(X)); test_preds['cat5'] = np.zeros(len(X_test))
cat5 = dict(iterations=5000,learning_rate=0.03,depth=5,l2_leaf_reg=2,
            min_data_in_leaf=10,subsample=0.8,colsample_bylevel=0.7,
            random_seed=SEED,task_type='CPU',verbose=0,eval_metric='RMSE',early_stopping_rounds=150)
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    m=CatBoostRegressor(**cat5)
    m.fit(X_arr[tr_i],y_arr[tr_i],eval_set=(X_arr[val_i],y_arr[val_i]),use_best_model=True)
    oof['cat5'][val_i] = m.predict(X_arr[val_i]); test_preds['cat5'] += m.predict(X_test_arr)/N_FOLDS
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['cat5'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['cat5'][val_i]):.4f} | {m.best_iteration_}itr")
print(f"CAT5 OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['cat5'])):.5f} R2={r2_score(y_arr,oof['cat5']):.4f}")

# CatBoost depth=6 (second best in V11 depth sweep)
print("\n=== CatBoost depth=6 ===")
...
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    ...
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['cat6'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['cat6'][val_i]):.4f} | {m.best_iteration_}itr")
print(f"CAT6 OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['cat6'])):.5f} R2={r2_score(y_arr,oof['cat6']):.4f}")

# CatBoost depth=8 (keep for ensemble diversity like V8)
print("\n=== CatBoost depth=8 ===")
...
for fold,(tr_i,val_i) in enumerate(kf.split(X_arr)):
    ...
    print(f"  F{fold+1}: RMSE={np.sqrt(mean_squared_error(y_arr[val_i],oof['cat8'][val_i])):.5f} R2={r2_score(y_arr[val_i],oof['cat8'][val_i]):.4f} | {m.best_iteration_}itr")
print(f"CAT8 OOF RMSE={np.sqrt(mean_squared_error(y_arr,oof['cat8'])):.5f} R2={r2_score(y_arr,oof['cat8']):.4f}")

# ExtraTrees (V8 had it as correction factor with negative coefficient)
print("\n=== Extra Trees ===")
...
# Also save cat6 only (best solo model)
for fname, preds in [
    (OUTPUT_FILE,                   stack_test),
    ("submission_v12_blend.csv",    blend_test),
    ("submission_v12_cat6.csv",     test_preds['cat6']),
]:
...
print("\n" + "="*60)
print("V12 SUMMARY (V8 features + Cat6 + Cat8)")
print("="*60)
...
print(f"  V10 Stack (LB 0.38240): RMSE=0.23438 R2=0.0364")
